...
open class WSTagsField: UIScrollView {
    ...
    open var prefix: String = "" {
        didSet {
            tagViews.forEach { $0.displayPrefix = self.isPrefixVisible ? self.prefix : "" }
        }
    }
    ...
    open var isPrefixVisible: Bool = false {
        didSet {
            tagViews.forEach { $0.displayPrefix = self.isPrefixVisible ? self.prefix : "" }
        }
    }
    ...
    open func addTag(_ tag: WSTag) {
        ...
        tagView.displayPrefix = self.isPrefixVisible ? self.prefix : ""
